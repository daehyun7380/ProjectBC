...
<% pageContext.setAttribute("nb","&nbsp;"); pageContext.setAttribute("sp"," "); %>
...
<body>
            ...
            <div class ="nav">
                ...
                <div class="column2">
					...
					<div class="b_pageNavi">
                    		<c:if test="${cph.showPrev}">
                    		<a href="<c:url value='/board/read_2${cph.csc.getQueryString(cph.beginPage-1)}&rec_num=${recDto.rec_num}'/>"><div class="b_preNext">이전</div></a>
                    		</c:if>
                    		<c:forEach var="i" begin="${cph.beginPage}" end="${cph.endPage}">
                    		<c:if test="${i == commPage}">
                    		<a href="<c:url value='/board/read_2${cph.csc.getQueryString(i)}&rec_num=${recDto.rec_num}'/>"><div id="select" class="b_pageNum">${i}</div></a>
                    		</c:if>
                    		<c:if test="${i != commPage}">
                    		<a href="<c:url value='/board/read_2${cph.csc.getQueryString(i)}&rec_num=${recDto.rec_num}'/>"><div class="b_pageNum">${i}</div></a>
                    		</c:if>
                    		</c:forEach>
                    		<c:if test="${cph.showNext}">
                    		<a href="<c:url value='/board/read_2${cph.csc.getQueryString(cph.endPage+1)}&rec_num=${recDto.rec_num}'/>"><div class="b_preNext">다음</div></a>
                    		</c:if>
                    	</div>
					<div class="b_commentflex">
					<div class="b_commentwritearea">
						<div class="b_commentwritewriter">${sessionScope.id}</div><textarea id="rec_comm_content" class="b_commentwritecontent" name="rec_comm_content"></textarea>
					</div>
	...
	<script>
		...
		window.location = "<c:url value='/board/list_2'/>?page=${page}&pageSize=${pageSize}";
		});
		...
		$("#writecommbtn").click(function(){
            ...
            // json 형식으로 데이터 set
            var params = {
                      rec_comm_content: $("#rec_comm_content").val(),
                      rec_num:$("#rec_num").val(),
                      page:${page},
            		  pageSize:${pageSize}
            }
            ...
            $.ajax({
                ...
                url : "/ch1/board/read_2",      // 컨트롤러에서 대기중인 URL 주소이다.
