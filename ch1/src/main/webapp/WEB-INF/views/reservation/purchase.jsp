<%@ page language="java" contentType="text/html; charset=UTF-8"
    pageEncoding="UTF-8"%>
<%@ taglib prefix="fmt" uri="http://java.sun.com/jsp/jstl/fmt" %>
<%@ taglib uri="http://java.sun.com/jsp/jstl/core" prefix="c" %>
<c:set var="path" value="${pageContext.request.contextPath }" />
<c:set var = "mypageLink" value="${sessionScope.id==null? '':'/myPage/myPage_main'}"/>
<c:set var = "mypage" value="${sessionScope.id==null? '':'마이 페이지'}"/>
<c:set var = "LoginOutlink" value="${sessionScope.id==null? '/logIn/logIn':'/logIn/logOut'}"/>
<c:set var = "LoginOut" value="${sessionScope.id==null? 'Login':'Logout'}"/>
<c:set var = "LoginOutlinkMypage" value="${sessionScope.id==null? '/BCsignup':''}"/>
<c:set var = "LoginMypage" value="${sessionScope.id==null? '회원가입':''}"/>
<c:set var="path" value="${pageContext.request.contextPath}"/>
<!DOCTYPE html>
<html>
<head>
   <meta charset="UTF-8">
    <title>비씨투어</title>
    <link rel="stylesheet" href="${path }/resources/CSS/BCtourMainStyle.css?awd">
    <link rel="icon" href="${path }/resources/img/상단로고.jpg" />
    <script src="https://ajax.googleapis.com/ajax/libs/jquery/3.3.1/jquery.min.js"></script>
</head>
<body>
    <body>
        <div class="main">
            <div class="allDiv">
                <div class="header">
                    <ul>  
	   	 				<li><a href="<c:url value='${LoginOutlinkMypage}'/>">${LoginMypage}</a></li>
	   	 				<li><a href="<c:url value='${LoginOutlink}'/>">${LoginOut}</a></li>  
                    </ul>
                </div>
                <div class="search" style="text-align: center;">
                    <div class="logoDiv">
                        <a href="<c:url value='/'/>"><img class="logo" src='data:image/png;base64,'/></a>
                    </div>
                    <form action="<c:url value='/searchProduct'/>">
                	<div class="searchEngine">
                        <input type="text" id="keyword" name="keyword" value="" placeholder="검색">
                        <button type="submit" class="b_searchBtn">검색</button>
                	</div>
                </form>
                    <div class="mypageDiv">
                        <ul class="mypage">
                            <c:if test="${sessionScope.id != 'admin'}">
                            <li><a href="<c:url value='${mypageLink}'/>">${mypage}</a></li>
                            </c:if>
                            <c:if test="${sessionScope.id == 'admin'}">
                            <li><a href="<c:url value='/myPage/manage'/>">고객 관리</a></li>
                            </c:if>
                            <li><a href="<c:url value='/board/list_2'/>">여행일지</a></li>
                        </ul>
                    </div>
                </div>
                <form id="form">
                    <div class="column2, b_purchasebox" >
                    <div class="b_purchaseContentBox">
                    <div class="b_purchaseContent">
                       <input type="hidden" name="pd_num" value=${info.pd_num}>
                       <input type="hidden" name="pd_city" value=${info.pd_city}>
                       <input type="hidden" name="pd_img" value=${info.pd_img}>
                       <input type="hidden" name="pd_subtitle" value=${info.pd_subtitle}>
                       <h1>상품명:&nbsp;&nbsp;<input id="pd_title" name="pd_title" class="b_pricebox" type="text" readOnly value="${info.pd_title}"></h1>
                       <hr style="width:400px;">
                       <h1>방문도시:&nbsp;&nbsp;<input id="pd_visitCity" name="pd_visitCity" class="b_pricebox" type="text" readOnly value="${info.pd_visitCity}"></h1>
                       <hr>
                       <div class="b_personCnt">
                       <h1>인원:</h1>&nbsp;&nbsp;&nbsp;&nbsp;
                       <div style="display:flex; align-items:center;">
                       <button type="button" id="plusQuantity" class="b_quantityBtn">+</button>
                       <input id="quantity" name="totalMember" class="b_quantityInput" type="text" size=4 value=1 autocomplete="off">
                       <button type="button" id="minusQuantity" class="b_quantityBtn">-</button>
                       </div>
                       </div>
                       <hr>
                       <h1>여행기간:&nbsp;&nbsp;<input id="pd_days" name="pd_days" class="b_pricebox" type="text" readOnly value="${info.pd_days}"></h1>
                       <hr>
                       <h1>교통수단:&nbsp;&nbsp;<input id="pd_transport" name="pd_transport" class="b_pricebox" type="text" readOnly value="${info.pd_transport}"></h1>
                       <hr>
                    </div>
                    <fmt:formatDate value="${info.pd_departDay}" pattern="yyyy.MM.dd" var="departDay" />
                    <fmt:formatDate value="${info.pd_deportDay}" pattern="yyyy.MM.dd" var="deportDay" />
                    <div class="b_purchaseContent">
                       <h1>출발일:&nbsp;&nbsp;<input id="pd_departDay" name="pd_departDay" class="b_pricebox" type="text" readOnly value="${departDay}"></h1>
                       <hr>
                       <h1>출발시간:&nbsp;&nbsp;<input id="pd_departStart" name="pd_departStart" class="b_pricebox" type="text" readOnly value="${info.pd_departStart}"></h1>
                       <hr>
                       <h1>도착시간:&nbsp;&nbsp;<input id="pd_departEnd" name="pd_departEnd" class="b_pricebox" type="text" readOnly value="${info.pd_departEnd}"></h1>
                       <hr>
                    </div>
                    <div class="b_purchaseContent">
                       <h1>복귀일:&nbsp;&nbsp;<input id="pd_deportDay" name="pd_deportDay" class="b_pricebox" type="text" readOnly value="${deportDay}"></h1>
                       <hr>
                       <h1>출발시간:&nbsp;&nbsp;<input id="pd_deportStart" name="pd_deportStart" class="b_pricebox" type="text" readOnly value="${info.pd_deportStart}"></h1>
                       <hr>
                       <h1>도착시간:&nbsp;&nbsp;<input id="pd_deportEnd" name="pd_deportEnd" class="b_pricebox" type="text" readOnly value="${info.pd_deportEnd}"></h1>
                       <hr>
                    </div>
                    </div>
                    <div class="b_purchaseCostBox">
                    <div class="b_purchaseContent">
                       <h1>총 결제금액(원):&nbsp;&nbsp;<input id="totalPrice" name="totalPrice" class="b_pricebox" type="text" readOnly value="${info.pd_price}"></h1>
                    </div>
                    <button type="submit" id="payBtn" class="b_payBtn">결제하기</button>
                    </div>
                    </div>
            	</form>
            </div>
        </div>
        <script>
        
        var maxQ = 5
        var price = ${info.pd_price}
        $(document).ready(function() {
			$("#plusQuantity").on('click', function() {
			      if ( $('#quantity').val() < maxQ ) {
			        $('#quantity').val(Number($('#quantity').val())+1),
			         $('#totalPrice').val(price*$('#quantity').val())
			      }     
				});
			});
        $(document).ready(function() {
			$("#minusQuantity").on('click', function() {
			      if ( $('#quantity').val() > 1 ) {
			        $('#quantity').val(Number($('#quantity').val())-1),
			         $('#totalPrice').val(price*$('#quantity').val())
			      }     
				});
			});
        
        $("#quantity").change(function(){
            if($('#quantity').val() > maxQ){
            	$('#quantity').val(maxQ)
            }
            if($('#quantity').val() < 1){
            	$('#quantity').val(1)
            }
            if(($('#quantity').val()*10)%10 != 0){
            	$('#quantity').val(1)
            }
            $('#totalPrice').val(price*$('#quantity').val())
        });
        
        document.getElementById('payBtn').addEventListener('click',e=>{
        if(!confirm("결제하시겠습니까?")) return "<c:url value='/purchase'/>";
    	var form = document.getElementById('form');
    	form.action="<c:url value='/purchase'/>";
    	form.method="post"
    	form.submit();
    	});
        
        </script>
</body>
</html>