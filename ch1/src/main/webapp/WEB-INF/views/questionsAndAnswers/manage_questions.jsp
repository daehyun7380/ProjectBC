...
<c:set var = "myPagePwd" value="${sessionScope.pwd==null? '/myPage/manage_pwdCheck':'/myPage/manage_managerInfo'}"/>
<!DOCTYPE html>
<html>
<head>
   <meta charset="UTF-8">
...
<body>
	<div class="main">
    	<div class="allDiv">
            ...
            <div class ="nav">
                <div id="column">
                    <div class="city">
                        <h2><a href="<c:url value='/myPage/manage_managerInfo'/>">고객정보</a></h2>
                    </div>
                    <div class="city">
                    ...
                    <div class="b_content">
                    	...
                    	<c:forEach var="i" items="${mQues}">
	                    	...
	                    	<div class="b_indextitle">
	                    		...
	                    		<div class="b_contentName"><a href="<c:url value='/myPage/manage_read_question?ques_num=${i.ques_num}&page=${page}&pageSize=${pageSize}'/>">${i.ques_title}</a></div>
	                    	...
	                    	<div class="b_indextitle">
	                    		...
	                    		<div class="b_contentName"><a href="<c:url value='/myPage/manage_read_answer?ans_num=${j.ans_num}&page=${page}&pageSize=${pageSize}'/>">└ ${j.ans_title}</a></div>
                    	...
                    	<div class="b_pageNavi">
                    		...
                    		<a href="<c:url value='/myPage/${manage_questions}${mph.sc.getQueryString(ph.beginPage-1)}'/>"><div class="b_preNext">이전</div></a>
                    		</c:if>
                    		<c:forEach var="i" begin="${mph.beginPage}" end="${mph.endPage}">
                    		<c:if test="${i == page}">
                    		<a href="<c:url value='/myPage/${manage_questions}${mph.sc.getQueryString(i)}'/>"><div id="select" class="b_pageNum">${i}</div></a>
                    		</c:if>
                    		<c:if test="${i != page}">
                    		<a href="<c:url value='/myPage/${manage_questions}${mph.sc.getQueryString(i)}'/>"><div class="b_pageNum">${i}</div></a>
                    		</c:if>
                    		</c:forEach>
                    		<c:if test="${mph.showNext}">
                    		<a href="<c:url value='/myPage/${manage_questions}${mph.sc.getQueryString(ph.endPage+1)}'/>"><div class="b_preNext">다음</div></a>
                    		</c:if>
                    	</div>
                    </div>
                    </form>
	...
	<script>
		...
		form.action="<c:url value='/myPage/manage_noAns'/>?page=${page}&pageSize=${pageSize}";
		form.method="Get"
		form.submit();
		});
