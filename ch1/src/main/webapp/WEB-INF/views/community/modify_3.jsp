...
<body>
                ...
                <div class="column2">
                	<div class="b_writecontent">
						<div class="b_writebtn">
							<input type="hidden" name="comm_num" value="${modi.comm_num}" readonly="readonly">
						...
						<div class="b_writecontentarea">
							<textarea name="comm_content" class="b_textarea" id="comm_content">${modi.comm_content}</textarea>
						</div>
					</div>
                </div>
                </form>
	...
	<script>
		...
		window.location = "<c:url value='/board/read_3'/>comm_num=${modi.comm_num}&page=${page}&pageSize=${pageSize}";
		});
	
		document.getElementById('writebtn').addEventListener('click',e=>{
		var form = document.getElementById('form');
		form.action="<c:url value='/board/modify_3'/>?page=${page}&pageSize=${pageSize}";
		form.method="post"
		form.submit();
		});
