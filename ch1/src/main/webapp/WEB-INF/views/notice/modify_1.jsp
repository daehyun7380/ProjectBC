...
<body>
                ...
                <div class="column2">
                	<div class="b_writecontent">
						<div class="b_writebtn">
							<input type="hidden" name="bno" value="${modi.bno}" readonly="readonly">
						...
						<div class="b_writetitlearea">
							<input class="b_writetitle" type="text" name="title" id="title" value="${modi.title}">
						</div>
						<div class="b_writecontentarea">
							<textarea name="content" class="b_textarea" id="content">${modi.content}</textarea>
						</div>
					</div>
                </div>
                </form>
	...
	<script>
		...
		window.location = "<c:url value='/board/read_1'/>bno=${modi.bno}&page=${page}&pageSize=${pageSize}";
		});
	
		document.getElementById('writebtn').addEventListener('click',e=>{
		var form = document.getElementById('form');
		form.action="<c:url value='/board/modify_1'/>?page=${page}&pageSize=${pageSize}";
		form.method="post"
		form.submit();
		});
